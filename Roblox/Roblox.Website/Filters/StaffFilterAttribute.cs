...
{
    private static HashSet<long> ownerUserIds { get; set; } = new();
    public static void Configure(IEnumerable<long> newOwnerUserIds)
    {
        ownerUserIds = new HashSet<long>(newOwnerUserIds);
        Console.WriteLine("[info] owners = {0}", string.Join(",", ownerUserIds));
    }
    ...
    public static async Task<IEnumerable<long>> GetStaff()
    {
        ...
        foreach (var id in ownerUserIds)
        {
            if (!list.Contains(id))
                list.Add(id);
        }
    ...
    {
        return ownerUserIds.Contains(userId);
    }
    ...
    public override async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        if (ownerUserIds.Contains(userInfo.userId))
        {
            await next();
            return;
        }
