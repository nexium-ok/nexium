...
namespace Roblox.Website.Controllers
{
    ...
    public class BypassController : ControllerBase
    {
        ...
        [HttpGet("promocodes/redeem")]
        public async Task<dynamic> RedeemPromoCode([Required] string code)
        {
            if (userSession == null)
                throw new RobloxException(401, 0, "Not logged in");

            var userId = userSession.userId;
            code = code.Trim().ToUpper();

            await using var promoLock = await Roblox.Services.Cache.redLock.CreateLockAsync(
                "PromoCodeRedeem:V1:" + userId + ":" + code, TimeSpan.FromSeconds(5));
            if (!promoLock.IsAcquired)
                throw new RobloxException(400, 0, "Please try again in a few seconds");

            var promocode = await services.users.db.QuerySingleOrDefaultAsync<PromoCodeEntry>(
            ...
            if (promocode.maxuses.HasValue && promocode.uses >= promocode.maxuses.Value)
                throw new RobloxException(400, 0, "This promocode has reached its maximum redemptions");

            var alreadyRedeemed = await services.users.db.ExecuteScalarAsync<int>(
            ...
            await services.users.InTransaction(async trx =>
            {
                var usersDb = services.users.db;

                await usersDb.ExecuteAsync(
                    "UPDATE promocodes SET uses = uses + 1 WHERE id = @id",
                    new { id = promocode.id }, trx);

                await usersDb.ExecuteAsync(
                    "INSERT INTO promocode_redemptions (promocode, user_id, asset_id, robux) VALUES (@promocodeID, @userId, @assetId, @robux)",
                    new
                    {
                ...
                if (promocode.asset_id.HasValue)
                {
                    await usersDb.ExecuteAsync(
                        ...
                        new { userId, assetId = promocode.asset_id.Value }, trx);
                }
                ...
                if (promocode.robux.HasValue && promocode.robux.Value > 0)
                {
                    await usersDb.ExecuteAsync(
                        ...
                        new { userId, amount = promocode.robux.Value }, trx);
                }
        ...
        private string GetSignature(string text, string privateKeyPath)
        {
            ...
            switch (details.assetType)
            {
                ...
                default:
                    ...
                    else
                    {
                        ...
                        // there was a vuln here, it let anyone download any asset, literally, even games
        ...
        try {
                ...
                return new
                {
                    jobId = "Test",
                    status = 2,
                    joinScriptUrl = "http://nexrev.org/Game/join.ashx?ticket=" + HttpUtility.UrlEncode(ticket) +
                                    "&job=Test",
                    authenticationUrl = "http://nexrev.org/Login/Negotiate.ashx",
        ...
        public async Task<MVC.IActionResult> JoinGame([Required, MVC.FromQuery] string ticket, [Required, MVC.FromQuery] string job)
        {
            FeatureFlags.FeatureCheck(FeatureFlag.GamesEnabled, FeatureFlag.GameJoinEnabled);
            var clientIp = GetIP();
            ...
            try
            {
                ...
                serverAddress = serverData.domain;
                placeId = serverData.placeId;
            }
            catch
            {
                placeId = ticketData.placeId;
                serverAddress = "127.0.0.1:53640";
            }
            ...
            accountAgeDays = DateTime.UtcNow.Subtract(userData.created).Days;
            ...
            //CORESCRIPT FIX PLEASE DONT TOUCH, ILL KILL YOU -Metano
            string newmembership = (int)membershipType switch
            {
                1 => "BuildersClub",
                2 => "TurboBuildersClub",
            ...
            string newbuildershit = ((int)uni.builderType == 1) ? "User" : "Group";
            // ALRIGHT FROM HERE ON YOU CAN TOUCH

            // 1. Create the anonymous object
            var joinData = new
            {
                ClientPort = 0,
                MachineAddress = ip,
                ...
                CreatorTypeEnum = newbuildershit,
                ...
                CharacterAppearance = "http://nexrev.org/Asset/CharacterFetch.ashx?userId=" + userId,
            };
            ...
            //script signature cuz im cool and for some reason ecs did use it (took the method straight from my php script :fire:)

            string jsonData = JsonSerializer.Serialize(joinData, jsonOptions);

            string dataToSign = "\r\n" + jsonData;

            string privateKeyPath = Path.Combine(Directory.GetCurrentDirectory(), "PrivateKey.pem"); 
            string signature = GetSignature(dataToSign, privateKeyPath);

            string finalResponse = $"--rbxsig%{signature}%\r\n{jsonData}";

            return Content(finalResponse, "text/plain");
        }

        [MVC.HttpPost("/apisite/gamejoin/v1/join-game")]
        public async Task<dynamic> JoinGameV1([Required, MVC.FromBody] JoinGameV1Request request)
        {
            FeatureFlags.FeatureCheck(FeatureFlag.GamesEnabled, FeatureFlag.GameJoinEnabled);
            var userId = safeUserSession.userId;
            var ip = GetIP();
            var result = await services.gameServer.GetServerForPlace(request.placeId);

            if (result.status != JoinStatus.Joining || result.job == null)
            {
                return new
                {
                    jobId = (string?)null,
                    status = (int)result.status,
                    joinScript = (object?)null,
                    message = "Waiting for server",
                };
            }

            var ticket = services.gameServer.CreateTicket(userId, request.placeId, ip);
            var serverData = services.gameServer.DecodeGameServerTicket(result.job);
            var joinScript = await BuildJoinScript(userId, ticket, serverData);
        ...
        private async Task<dynamic> BuildJoinScript(long userId, string ticket, GameServerTicketJwt serverData)
{
    var userInfo = await services.users.GetUserById(userId);
    var accountAgeDays = DateTime.UtcNow.Subtract(userInfo.created).Days;
    var statusIs18OrOver = await services.users.Is18Plus(userId);
    ...
    return new
    {
        ...
        [MVC.HttpPost("/universes/validate-place-join")]
        [MVC.HttpGet("/universes/validate-place-join")]
        public dynamic ValidatePlaceJoin()
        {
            return "true";
        }
        ...
        [HttpGetBypass("/apisite/clientsettings/Setting/QuietGet/RCCServiceNEXIUMACCESSKEY#$")]
        [HttpGetBypass("/apisite/clientsettings/Setting/QuietGet/NEXIUMACCESSKEY#$")]
        ...
        {
            var applicationName = HttpContext.Request.GetEncodedUrl();
        ...
        public async Task<dynamic> IncrementPersistence(long placeId, string key, string type, string scope, string target, int value)
        {
            ...
            var newValue = await ServiceProvider.GetOrCreate<DataStoreService>()
                .Increment(placeId, target, type, scope, key, value);
        ...
        public async Task<dynamic> GetSortedPersistenceValues(long placeId, string type, string scope, string key, int pageSize, bool ascending, int inclusiveMinValue = 0, int inclusiveMaxValue = 0)
        {
            ...
            var entries = await ServiceProvider.GetOrCreate<DataStoreService>()
                .GetSortedValues(placeId, scope, key, pageSize, ascending, inclusiveMinValue, inclusiveMaxValue);
        ...
        public async Task<dynamic> Set(long placeId, string key, string type, string scope, string target, int valueLength, [Required, MVC.FromBody] SetRequest request)
        {
            ...
            return new
            {
                ...
                [HttpPostBypass("persistence/remove")]
        public async Task<dynamic> RemovePersistence(long placeId, string key, string type, string scope, string target)
        {
            // remove?placeId=%i&key=%s&type=%s&scope=%s&target=
            if (!IsRcc())
                throw new RobloxException(400, 0, "BadRequest");

            var removedValue = await ServiceProvider.GetOrCreate<DataStoreService>()
                .Remove(placeId, type, scope, target, key);
