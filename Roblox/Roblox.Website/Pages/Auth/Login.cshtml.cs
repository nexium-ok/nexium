...
public class Login : RobloxPageModel
{
    ...
    public bool needsTwoStepCode { get; set; }
    ...
    public async Task<IActionResult> OnPost()
    {
        ...
        if (is2FAEnabled)
        {
            if (string.IsNullOrWhiteSpace(twoStepCode))
            {
                needsTwoStepCode = true;
                errorMessage = "Enter the 6-digit code from your authenticator app.";
                return new PageResult();
            }

            var codeValid = await services.twoFactor.VerifyCode(userId, twoStepCode);
            if (!codeValid)
            {
                needsTwoStepCode = true;
                errorMessage = "Invalid 2FA verification code. Please try again.";
                return new PageResult();
            }
        }
