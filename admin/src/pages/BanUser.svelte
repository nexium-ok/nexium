<script lang="ts">
	...
	const quickFillReasons: { name: string; text: string }[] = [
		{
			name: 'TOS Violation',
			text: 'This account has been closed due to violating Nexrev terms of service.',
		},
		{
			name: 'Bad Username',
			text: 'Your username is inappropriate for Nexrev.',
		},
		// fuck this shit, bad username is enough already, and you can type your own reason.
		//{
		//	name: 'Bad Username (Privacy Issue)',
		//	text: 'Your username is not appropriate for Nexrev due to privacy concerns. ',
		//},
		{
			name: 'Spam',
			text: 'Do not repeatedly post spam chat or content in Nexrev.',
		},
		{
		...
		{
			name: 'Hate Speech',
			text: 'This content is not appropriate. Hate speech is not permitted on Nexrev.',
		},
		{
			name: 'Real-Life Information',
			text: 'Do not ask for or give out personal, real-life, or private information on Nexrev.',
		},
		{
		...
		{
			name: 'USDer',
			text: 'Your account has been moderated for buying, selling, or trading Robux or virtual Nexrev items outside of the Nexrev website.',
		},
		{
			name: 'Pois Lims',
			...
			<div class="row mt-4">
				<div class="col-12 col-lg-3">
					<select class="form-control" bind:value={expires}>
						<option value="permanent">Terminate</option>
			...
			<button
				...
				on:click={(e) => {
					...
					request
						...
						.catch((e) => {
							errorMessage = (e && e.message) || 'Something went wrong. Please try again.';
						})
						.finally(() => {
							disabled = false;
						});
