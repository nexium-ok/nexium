...
public class RobloxApi
{
    private static HttpClient _client { get; } = new(new HttpClientHandler()
    {
        AutomaticDecompression = DecompressionMethods.All,
            ...
            public async Task<ProductInfoWithAssetDelivery> GetProductInfoAssetDelivery(long assetId, string openCloudApiKey)
    {
        // Literally all it gets is the "assetTypeId". Everything else is blank.
        if (string.IsNullOrWhiteSpace(openCloudApiKey))
            throw new Exception("RobloxOpenCloudApiKey is not configured. As of April 2025, Roblox requires " +
                                 "an Open Cloud API key (with the legacy-asset:manage scope) to fetch asset delivery info. " +
                                 "Set RobloxOpenCloudApiKey in appsettings.json.");
        ...
        while (true)
        {
            ...
            request.Headers.Add("x-api-key", openCloudApiKey);
            var response = await _client.SendAsync(request, cancel.Token);

            if (response.StatusCode is HttpStatusCode.TooManyRequests)
            {
                if (attempt >= maxAttempts)
                    throw new Exception($"Roblox rate-limited GetProductInfoAssetDelivery {maxAttempts} times in a row (429) for asset {assetId}.");
            ...
            if (response.StatusCode is HttpStatusCode.Unauthorized or HttpStatusCode.Forbidden)
                throw new Exception("Roblox rejected the Open Cloud API key (status " + response.StatusCode +
                                     ") in GetProductInfoAssetDelivery. Check that RobloxOpenCloudApiKey is valid and has the legacy-asset:manage scope.");
            if (!response.IsSuccessStatusCode)
            {
                throw new Exception("Unexpected status code from AssetDelivery: " + response.StatusCode);
            }

            var str = await response.Content.ReadAsStringAsync(cancel.Token);
            var json = JsonSerializer.Deserialize<AssetDeliveryResponse>(str);
            if (json == null)
   ...
   public async Task<Stream> GetAssetContent(long assetId, string openCloudApiKey)
    {
        if (string.IsNullOrWhiteSpace(openCloudApiKey))
            throw new Exception("RobloxOpenCloudApiKey is not configured " +
                                 "an Open Cloud API key is needed to fetch asset content. " +
                                 "Set RobloxOpenCloudApiKey in appsettings.json.");
        ...
        while (true)
        {
            ...
            {
                if (attempt >= maxAttempts)
                    throw new Exception($"Roblox rate-limited this request {maxAttempts} times in a row (429). " +
                                         "Try again or just get a different api key");
            ...
            if (result.StatusCode is HttpStatusCode.Unauthorized or HttpStatusCode.Forbidden)
                throw new Exception("Roblox rejected the Open Cloud API key (status " + result.StatusCode +
                                     "). Check that RobloxOpenCloudApiKey is valid and has the legacy-asset:manage scope.");
            if (!result.IsSuccessStatusCode)
                throw new Exception("Unexpected response from Roblox: " + result.StatusCode);
