...
public class GroupsService : ServiceBase, IService
{
        ...
        public async Task RenameGroup(long groupId, string newName)
    {
        ...
        if (await IsGroupNameTaken(newName))
            throw new ArgumentException("That group name is already taken");

        await db.ExecuteAsync(
            "UPDATE \"group\" SET name = :name, updated_at = now() WHERE id = :id",
            new
            {
                id = groupId,
                name = newName,
    ...
    public async Task DeleteGroupPermanently(long groupId)
    {
        ...
        await db.ExecuteAsync("DELETE FROM group_wall WHERE group_id = :id", new { id = groupId });
        await db.ExecuteAsync("DELETE FROM group_status WHERE group_id = :id", new { id = groupId });
        await db.ExecuteAsync("DELETE FROM group_audit_log WHERE group_id = :id", new { id = groupId });
        await db.ExecuteAsync("DELETE FROM group_icon WHERE group_id = :id", new { id = groupId });
        await db.ExecuteAsync("DELETE FROM group_settings WHERE group_id = :id", new { id = groupId });
        await db.ExecuteAsync("DELETE FROM group_social_link WHERE group_id = :id", new { id = groupId });
        await db.ExecuteAsync("DELETE FROM group_economy WHERE group_id = :id", new { id = groupId });

        await db.ExecuteAsync(
            "DELETE FROM group_role_permission WHERE group_role_id IN (SELECT id FROM group_role WHERE group_id = :id)",
            new { id = groupId });
        await db.ExecuteAsync(
            "DELETE FROM group_user WHERE group_role_id IN (SELECT id FROM group_role WHERE group_id = :id)",
            new { id = groupId });
        await db.ExecuteAsync("DELETE FROM group_role WHERE group_id = :id", new { id = groupId });

        await db.ExecuteAsync("DELETE FROM \"group\" WHERE id = :id", new { id = groupId });
    }
