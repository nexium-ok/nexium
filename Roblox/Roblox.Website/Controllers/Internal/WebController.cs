...
public class WebController : ControllerBase
{
    ...
    // upload thumbnail endpoint, stolen with love from bubbablox, hope this doesnt have vulns
    ...
    public async Task<IActionResult> UploadGameIcon([Required] long placeId, [Required] IFormFile file)
    {
        ...
        // check balance
        var economy = await services.economy.GetUserBalance(safeUserSession.userId);
        if (economy.robux < 25)
            throw new BadRequestException(0, "You need at least 25 Robux to upload a game icon");

        stream.Position = 0;
        await services.assets.CreateGameIcon(placeId, stream);

        // take away 25 Robux
        await services.economy.DecrementCurrency(safeUserSession.userId, CurrencyType.Robux, 25);
        await services.users.InsertAsync("user_transaction", new
        {
            ...
            user_id_one = safeUserSession.userId,
            user_id_two = 1,
        });
