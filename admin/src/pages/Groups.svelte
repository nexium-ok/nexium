            ...
            {#if groupData}
                <div class="row">
                    ...
                    <div class="col-8">
                        ...
                        <button disabled={disabled} class="btn btn-outline-danger" on:click={() => {
                            ...
                            request.post('/groups/reset?groupId=' + groupData.info.id).then(result => {
                                ...
                                disabled = false;
                                                ...
                                                <Permission p="RenameGroup">
                            <div class="row mt-3">
                                <div class="col-8">
                                    <input disabled={disabled} bind:value={newGroupName} type="text" class="form-control" placeholder="New group name" />
                                </div>
                                <div class="col-4">
                                    <button disabled={disabled || !newGroupName} class="btn btn-outline-primary" on:click={() => {
                                        disabled = true;
                                        request.patch('/groups/rename?groupId=' + groupData.info.id + '&newName=' + encodeURIComponent(newGroupName)).then(() => {
                                            groupData.info.name = newGroupName;
                                            groupData = {...groupData};
                                            newGroupName = '';
                                        }).catch(e => {
                                            feedback = e.message;
                        ...
                        <Permission p="DeleteGroupPermanently">
                            <button disabled={disabled} class="btn btn-danger mt-3" on:click={() => {
                                if (prompt(`Type the group's name ("${groupData.info.name}") to permanently delete it. This cannot be undone.`) !== groupData.info.name) {
                                    return;
                                }
                                disabled = true;
                                request.delete('/groups/delete?groupId=' + groupData.info.id).then(() => {
                                    feedback = undefined;
                                    groupData = undefined;
                                    loadGroups();
                                }).catch(e => {
