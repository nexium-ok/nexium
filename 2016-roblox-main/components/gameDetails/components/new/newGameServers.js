...
const ServerEntry = props => {
  ...
  return <div className={s.serverContainer}>
    ...
    <div className={s.callsToAction}>
      ...
      <ActionButton label='Join' buttonStyle={buttonStyles.newCancelButton} className={s.joinButton} disabled={props.canJoin === false} tooltipText='This game is private. Only the owner can join.' onClick={onJoinButtonClick} />
    </div>
...
const GameServers = props => {
  ...
  const privacyRaw = store.universeDetails?.privacyType ?? store.placeDetails?.privacyType ?? store.details?.privacyType;
  const isPrivateGame = privacyRaw === 3 || String(privacyRaw || '').toLowerCase() === 'private';
  const isOwner = auth.isAuthenticated && (creatorType === 'User' || creatorType === 1) && Number(creatorId) === Number(auth.userId);
  const canJoinGame = !isPrivateGame || isOwner;
  ...
  return <div className={`${s.subSectionContainer}`}>
    ...
    </div>
    {
      friendServers && friendServers.length > 0 && friendServers.map(v => {
        return <ServerEntry key={v.Guid} {...v} canJoin={canJoinGame} />
      })
      ||
