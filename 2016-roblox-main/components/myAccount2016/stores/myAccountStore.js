...
import { get2FASetup } from "../../../services/auth";
...
const MyAccountStore = createContainer(() => {
  ...
  const [twoFactorEnabled, setTwoFactorEnabled] = useState(null); // null = loading
  const [twoFactorSetupUrl, setTwoFactorSetupUrl] = useState(null);
  /**
  ...
  useEffect(() => {
    ...
    get2FASetup().then(d => {
      ...
      setTwoFactorSetupUrl(d.setupUrl);
    })
  }, []);
