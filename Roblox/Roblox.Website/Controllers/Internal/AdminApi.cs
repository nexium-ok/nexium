...
private bool IsLoggedIn()
    ...
    [HttpPost("promocodes/create"), StaffFilter(Access.ManagePromocodes)]
    ...
    [HttpGet("promocodes/list"), StaffFilter(Access.ManagePromocodes)]
    ...
    [HttpGet("promocodes/redemptions"), StaffFilter(Access.ManagePromocodes)]
    ...
    [HttpPost("promocodes/toggle-active"), StaffFilter(Access.ManagePromocodes)]
    ...
    [HttpDelete("promocodes/delete"), StaffFilter(Access.ManagePromocodes)]
        ...
        [FromServices] NpgsqlConnection db)
    {
        ...
        await db.ExecuteAsync("DELETE FROM promocode_redemptions WHERE promocode = @id", new { id = request.promocodeID });
        await db.ExecuteAsync("DELETE FROM promocodes WHERE id = @id", new { id = request.promocodeID });
    ...
    [HttpPost("user/disable-2fa"), StaffFilter(Access.DisableTwoFactor)]
    public async Task DisableTwoFactor([Required, FromBody] UserIdRequest request)
    {
        if (await IsStaff(request.userId) && !StaffFilter.IsOwner(userSession.userId))
            throw new StaffException("Bad user id");
        await services.twoFactor.Disable2FA(request.userId);
    }
...
-The Roblox Team"
    ...
    public async Task UpdateAssetInfo([Required, FromBody] UpdateNameRequest request)
    {
        ...
        if (request.newDescription != null && request.newDescription.Length > 1000)
        ...
        await db.ExecuteAsync(
            ...
            new
            {
                ...
                new_description = request.newDescription ?? assetInfo.description,
            });
    }

    [HttpDelete("asset/delete"), StaffFilter(Access.DeleteCatalogAsset)]
    public async Task DeleteCatalogAsset([Required, FromQuery] long assetId)
    {
        await services.assets.GetAssetCatalogInfo(assetId); // throws RecordNotFoundException if it doesn't exist
        await services.assets.DeleteAsset(assetId);
    }
    ...
    public async Task<dynamic> CopyAssetFromRoblox([Required, FromBody] CopyAssetRequest request)
    {
        ...
        var content = await services.robloxApi.GetAssetContent(request.assetId, Roblox.Configuration.RobloxOpenCloudApiKey);
   ...
   // stolen frkm bubba blox thanks harryzawg
    ...
    public async Task<dynamic> CopyBundle(long bundleId)
    {
        ...
        if (alreadyExists._total > 0 && alreadyExists.data != null)
        {
            ...
            {
                var info = await services.assets.GetAssetCatalogInfo(item.id);
                if (info.assetType == Type.Package && info.name == details.name)
                {
                    throw new StaffException("It looks like this bundle already exists: AssetID=" + info.id);
                }
            }
        }
        ...
        foreach (var item in details.items)
        {
            ...
            var content = await services.robloxApi.GetAssetContent(item.id, Roblox.Configuration.RobloxOpenCloudApiKey);
    ...
    public async Task<dynamic> CreateAsset([Required, FromForm] CreateAssetRequest request)
    {
        ...
        if (isPackage)
        {
            ...
            var optionalOneOf = new List<Type>() {Type.LeftArm, Type.LeftLeg, Type.RightLeg, Type.RightArm, Type.Torso, Type.Head, Type.Gear, Type.Shirt, Type.Pants, Type.Face};
                        var optionalCanHaveMoreThanOne = new List<Type>() {Type.Hat, Type.HairAccessory, Type.ShoulderAccessory, Type.BackAccessory, Type.FrontAccessory, Type.WaistAccessory, Type.NeckAccessory, Type.FaceAccessory};
            // optional
    ...
    public async Task ResetGroup(long groupId)
    {
        ...
        [HttpPatch("groups/rename"), StaffFilter(Access.RenameGroup)]
    public async Task RenameGroup(long groupId, string newName)
    {
        var group = await services.groups.GetGroupById(groupId);

        try
        {
            await services.groups.RenameGroup(groupId, newName);
        }
        catch (ArgumentException e)
        {
            throw new StaffException(e.Message);
        }

        await db.ExecuteAsync(
            "INSERT INTO moderation_rename_group (group_id, actor_id, old_name, new_name) VALUES (:group_id, :actor_id, :old_name, :new_name)",
            new
            {
                group_id = groupId,
                actor_id = userSession.userId,
    ...
    [HttpDelete("groups/delete"), StaffFilter(Access.DeleteGroupPermanently)]
    public async Task DeleteGroupPermanently(long groupId)
    {
        var group = await services.groups.GetGroupById(groupId); // throws RecordNotFoundException if it doesn't exist

        await db.ExecuteAsync(
            "INSERT INTO moderation_delete_group (group_id, group_name, actor_id) VALUES (:group_id, :group_name, :actor_id)",
            new
            {
                group_id = groupId,
        ...
        await services.groups.DeleteGroupPermanently(groupId);
    }
