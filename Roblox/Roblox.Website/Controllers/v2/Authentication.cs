...
public class AuthenticationControllerV2 : ControllerBase
{
    ...
    public async Task<dynamic> Get2FASetup()
    {
        ...
        var token = await services.twoFactor.GetOrCreateSetupToken(safeUserSession.userId);
        return new
        {
            enabled = false,
            setupUrl = "/auth/2fa/" + token,
        };
    }
    ...
    public async Task Login([Required, FromBody] LoginRequest request)
    {
        ...
        var is2FAEnabled = await services.twoFactor.IsEnabled(userId);
        if (is2FAEnabled)
        {
            if (string.IsNullOrWhiteSpace(request.twoStepCode))
            {
                throw new ForbiddenException(2, "Two-step verification code required.");
            }

            var codeValid = await services.twoFactor.VerifyCode(userId, request.twoStepCode);
            if (!codeValid)
            {
                throw new ForbiddenException(3, "Invalid two-step verification code. Please try again.");
            }
        }
